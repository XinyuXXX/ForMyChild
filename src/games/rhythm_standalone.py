...
class RhythmGame:
    ...
    def _init_rhythms(self):
        ...
        # 根据难度选择节奏模式（1-10级）
        difficulty_patterns = {
            1: all_patterns[:1],    # 超简单：只有全音符
            2: all_patterns[:2],    # 很简单：全音符、二分音符
            3: all_patterns[:3],    # 简单：+四分音符
            4: all_patterns[:4],    # 较易：+八分音符
            5: all_patterns[:5],    # 适中：+附点二分音符
            6: all_patterns[:6],    # 稍难：全部基础节奏
            7: all_patterns[:6],    # 困难：全部节奏，速度加快
            8: all_patterns[:6],    # 很困难：全部节奏，更快速度
            9: all_patterns[:6],    # 超困难：全部节奏，很快速度
            10: all_patterns[:6]    # 极限：全部节奏，极快速度
        }
        
        self.available_rhythms = difficulty_patterns.get(self.difficulty, all_patterns[:3])
        
        # 根据难度调整播放速度（影响节奏难度）
        self.tempo_multiplier = 1.0
        if self.difficulty >= 7:
            self.tempo_multiplier = 1.2
        if self.difficulty >= 8:
            self.tempo_multiplier = 1.4
        if self.difficulty >= 9:
            self.tempo_multiplier = 1.6
        if self.difficulty >= 10:
            self.tempo_multiplier = 2.0
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if not self.initialized:
            ...
            loading_text = loading_font.render("游戏加载中...", True, COLORS['text'])
    ...
    def _draw_rhythm_question(self, screen: pygame.Surface):
        ...
        # 绘制节奏
        x_step = (measure_x_end - measure_x_start) / max(total_beats, 1)
